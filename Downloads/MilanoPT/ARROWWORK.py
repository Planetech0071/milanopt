        ...
        .then(data => {
          ...
          if (data.line_stops_with_wait_times && data.line_stops_with_wait_times.length > 0) {
            ...
            if (closestVehicleStop && minWaitMinutes !== Infinity) {
              ...
              const vehicleRadiusMeters = 8.75; // Half of iconSize (17.5px / 2) converted to approximate meters
              const arrowLatLng = getPointOnPolylineSimple(
                currentLinePaths,
                newTargetLatLon,
                -vehicleRadiusMeters, // Use actual vehicle radius
                map
              );
                ...
                data.vehicles.forEach(vehicle => {
                  ...
                  if (approachingStation) {
                    ...
                    const vehicleArrowLatLng = getPointOnPolylineSimple(
                      currentLinePaths,
                      projectedLatLon,
                      -vehicleRadiusMeters, // Use consistent radius
                      map
                    );
