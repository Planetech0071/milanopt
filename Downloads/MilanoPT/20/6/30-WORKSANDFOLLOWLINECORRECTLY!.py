...
try:
    print("Attempting to load line stations memory from FINAL.json...")
    with open('FINAL.json', 'r', encoding='utf-8') as f:
        line_stations_memory = json.load(f)
    print(f"Loaded stations memory for {len(line_stations_memory)} line entries from FINAL.json.")
except (FileNotFoundError, json.JSONDecodeError) as e:
    print(f"Error loading stations memory from FINAL.json ({e}). Exiting...")
    raise
...
def get_line_vehicle_data():
    ...
    if line_info:
        ...
        stop_ids = [stop['stop_id'] for stop in stops_data]
        
        # Create a mapping of stop IDs to their sequence in the line
        stop_sequence = {stop['stop_id']: idx for idx, stop in enumerate(stops_data)}
...
function animateVehicles() {
  ...
  stops.forEach(stop => {
    ...
    marker.on('click', function(e) {
      ...
      fetch(`/station_lines?stop_id=${stop.stop_id}`)
        ...
        .then(data => {
          ...
          // Then get wait times
          fetch(`/wait_time?stop_id=${stop.stop_id}`)
            .then(response => response.json())
            .then(waitData => {
  ...
  function findPolylineSegmentBetweenStops(allPaths, startStop, endStop) {
    console.log(`🛤️ Finding polyline segment from ${startStop.name} to ${endStop.name}`);
    
    let bestSegment = [];
    let minTotalDistance = Infinity;
    
    // Check each path to find the one that contains both stops
    for (const path of allPaths) {
        ...
        // Find closest points on this path for both stops
        path.forEach((point, idx) => {
            const startDist = Math.abs(point.Y - startStop.lat) + Math.abs(point.X - startStop.lon);
            const endDist = Math.abs(point.Y - endStop.lat) + Math.abs(point.X - endStop.lon);
        ...
        // Make sure we found both points and they're in the right order
        if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
            const totalDist = minStartDist + minEndDist;
            if (totalDist < minTotalDistance) {
                minTotalDistance = totalDist;
                // Extract the segment between the two stops
                bestSegment = path.slice(startIdx, endIdx + 1);
            }
        }
    }
    ...
    if (bestSegment.length > 0) {
        console.log(`  ✅ Found polyline segment with ${bestSegment.length} points`);
        return bestSegment;
    }
    
    // Fallback: return straight line if no polyline found
    console.log(`  ⚠️ No polyline segment found, using straight line`);
    return [
        {Y: startStop.lat, X: startStop.lon},
        {Y: endStop.lat, X: endStop.lon}
    ];
  }
                    ...
                    data.vehicles.forEach((vehicle, vehicleIdx) => {
                        ...
                        if (vehicleIcon) {
                            ...
                            if (currentLinePaths.length > 0 && nextStop) {
                                ...
                                const animationPath = findPolylineSegmentBetweenStops(
                                    currentLinePaths, 
                                    nextStop, 
                                    {lat: vehicle.lat, lon: vehicle.lon, name: vehicle.stop_name}
                                );
...
function findNextStationByWaitTime(currentVehicle, allStops, allPaths) {
  ...
  function interpolateOnPolyline(polyline, progress, mapInstance) {
    if (polyline.length < 2) {
        return [polyline[0].Y, polyline[0].X];
    }
    ...
    // Find target distance along the polyline
    ...
    for (let i = 0; i < segmentLengths.length; i++) {
        ...
        if (accumulatedDistance + segmentLength >= targetDistance) {
            ...
            const lat = p1.Y + (p2.Y - p1.Y) * segmentProgress;
            const lon = p1.X + (p2.X - p1.X) * segmentProgress;
