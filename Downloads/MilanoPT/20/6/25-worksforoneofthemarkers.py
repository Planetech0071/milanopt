...
<script>
  ...
  function animateVehicles() {
      ...
      vehicleAnimations.forEach((animData, vehicleId) => {
          if (animData.isAnimating && animData.polyline) {
              ...
              if (progress >= 1) {
                  ...
                  const stopsList = data.line_stops_with_wait_times; // You need to have this accessible
                  let currIdx = stopsList.findIndex(s => s.id === animData.currStop.id);
                  let nextIdx = (currIdx + 1) % stopsList.length;
                  let newPrev = animData.currStop;
                  let newCurr = stopsList[nextIdx];
                  // Update animation data for next segment
                  animData.prevStop = newPrev;
                  animData.currStop = newCurr;
                  animData.polyline = findPolylineSegment(currentLinePaths, newPrev, newCurr);
                  animData.startTime = Date.now();
                  animData.duration = animationDuration; // or recalculate based on distance
    ...
    const updateVehiclePositions = () => {
        ...
        fetch(`/get_line_vehicle_data?line_number=${lineNumber}`)
            ...
            .then((data) => {
                ...
                console.log('  - Has line_stops_with_wait_times:', !!data.line_stops_with_wait_times);
                console.log('  - Array length:', data.line_stops_with_wait_times?.length || 0);
                
                if (data.line_stops_with_wait_times) {
                    console.log('📋 All stops with wait times:');
                    data.line_stops_with_wait_times.forEach((stop, idx) => {
                        console.log(`  ${idx}: ${stop.name} - Wait: "${stop.wait_message}" - Parsed: ${parseWaitMinutes(stop.wait_message)} mins`);
                    });
                }
                ...
                vehicleAnimations.clear();
                console.log('✨ Created new vehicle layer');
                ...
                if (data.line_stops_with_wait_times && data.line_stops_with_wait_times.length > 0) {
                    console.log('🔍 Analyzing stops for arriving vehicles...');
                    
                    // Find all stops with different wait time conditions
                    const allStopsAnalysis = data.line_stops_with_wait_times.map(stop => {
                        const parsedMins = parseWaitMinutes(stop.wait_message);
                        return {
                            name: stop.name,
                            wait_message: stop.wait_message,
                            parsed_minutes: parsedMins,
                            is_arriving: parsedMins === 0,
                            is_recalculating: parsedMins === -1,
                            has_service: parsedMins !== Infinity
                        };
                    });
                    
                    console.log('📊 STOPS ANALYSIS:', allStopsAnalysis);
                    
                    // Find stops with vehicles arriving (wait time == 0)
                    const arrivingStops = data.line_stops_with_wait_times.filter(stop => {
                        const parsed = parseWaitMinutes(stop.wait_message);
                        console.log(`🚌 Checking stop ${stop.name}: wait="${stop.wait_message}" -> parsed=${parsed} -> arriving=${parsed === 0}`);
                        return parsed === 0;
                    });
                    
                    console.log('🎯 ARRIVING STOPS FOUND:', arrivingStops.length);
                    arrivingStops.forEach((stop, idx) => {
                        console.log(`  ${idx + 1}. ${stop.name} - "${stop.wait_message}"`);
                    });
                    
                    // Also check for stops with short wait times (1-2 minutes) for debugging
                    const shortWaitStops = data.line_stops_with_wait_times.filter(stop => {
                        const parsed = parseWaitMinutes(stop.wait_message);
                        return parsed > 0 && parsed <= 2;
                    });
                    
                    console.log('⏰ SHORT WAIT STOPS (1-2 mins):', shortWaitStops.length);
                    shortWaitStops.forEach((stop, idx) => {
                        console.log(`  ${idx + 1}. ${stop.name} - "${stop.wait_message}" (${parseWaitMinutes(stop.wait_message)} mins)`);
                    });

                    // Highlight approaching stops (red markers)
                    arrivingStops.forEach((stop, idx) => {
                        console.log(`🔴 Creating highlight marker ${idx + 1} for ${stop.name} at [${stop.lat}, ${stop.lon}]`);
                        ...
                        const highlightMarker = L.circleMarker([stop.lat, stop.lon], {
                            ...
                            opacity: 1,
                            fillOpacity: 1
                        }).bindPopup(`🚌 Approaching: ${stop.name}`);
                    ...
                    // Create vehicle markers for arriving stops
                    console.log('🚗 Creating vehicle markers...');
                    arrivingStops.forEach((stop, vehicleIdx) => {
                        console.log(`\n🚗 PROCESSING VEHICLE ${vehicleIdx + 1} for stop: ${stop.name}`);
                        
                        // Find the index of the approaching stop in the full list
                        const stopIdx = data.line_stops_with_wait_times.findIndex(s => s.id === stop.id);
                        console.log(`  📍 Stop index in route: ${stopIdx}`);
                        
                        // Find the previous stop
                        let prevIdx = stopIdx - 1;
                        if (prevIdx < 0) {
                            prevIdx = data.line_stops_with_wait_times.length - 1; // Wrap to end
                            console.log(`  🔄 Wrapped to last stop (circular route)`);
                        }
                        
                        const prevStop = data.line_stops_with_wait_times[prevIdx];
                        console.log(`  📍 Previous stop: ${prevStop.name} at index ${prevIdx}`);
                        
                        // Calculate bearing from previous stop to current stop
                        let bearing = 0;
                        if (prevStop && (prevStop.lat !== stop.lat || prevStop.lon !== stop.lon)) {
                            const lat1 = prevStop.lat * Math.PI / 180;
                            const lon1 = prevStop.lon * Math.PI / 180;
                            const lat2 = stop.lat * Math.PI / 180;
                            const lon2 = stop.lon * Math.PI / 180;
                            ...
                            console.log(`  🧭 No bearing calculation (same position or no previous stop)`);
                        }
                        
                        // Calculate vehicle position
                        const stopLatLon = [stop.lat, stop.lon];
                        const prevLatLon = [prevStop.lat, prevStop.lon];
                        
                        console.log(`  📐 Stop position: [${stopLatLon[0]}, ${stopLatLon[1]}]`);
                        console.log(`  📐 Previous stop position: [${prevLatLon[0]}, ${prevLatLon[1]}]`);
                        
                        // Calculate distance between stops
                        const totalDist = map.distance(
                            L.latLng(prevLatLon[0], prevLatLon[1]), 
                            L.latLng(stopLatLon[0], stopLatLon[1])
                        );
                        console.log(`  📏 Distance between stops: ${totalDist.toFixed(2)} meters`);
                        
                        // For "in arrivo" vehicles, place them close to the stop
                        const distanceFromStop = 50; // 50 meters before the stop
                        console.log(`  🎯 Placing vehicle ${distanceFromStop} meters before stop`);
                        
                        // Check if we have polyline paths
                        console.log(`  🛣️ Available paths: ${currentLinePaths.length}`);
                        if (currentLinePaths.length > 0) {
                            console.log(`  🛣️ Total path segments: ${currentLinePaths.reduce((sum, path) => sum + path.length, 0)}`);
                        }
                        
                        let vehiclePosition;
                        if (currentLinePaths.length > 0) {
                            console.log(`  🔄 Using polyline positioning`);
                            vehiclePosition = getPointOnPolylineSimple(
                                currentLinePaths,
                                stopLatLon,
                                distanceFromStop,
                                map
                            );
                            console.log(`  📍 Polyline position: [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                        } else {
                            console.log(`  ⚠️ No polyline data, using simple interpolation`);
                            // Simple interpolation between previous stop and current stop
                            const t = 0.1; // 10% of the way from current to previous
                            vehiclePosition = [
                                stopLatLon[0] + t * (prevLatLon[0] - stopLatLon[0]),
                                stopLatLon[1] + t * (prevLatLon[1] - stopLatLon[1])
                            ];
                            console.log(`  📍 Interpolated position: [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                        }
                        
                        // Create vehicle icon
                        console.log(`  🎨 Creating vehicle icon for line ${lineNumber} with bearing ${bearing.toFixed(2)}°`);
                        ...
                        if (vehicleIcon) {
                            ...
                            const popupContent = `🚌 Live Vehicle: ${lineNumber}<br>📍 Approaching: ${stop.name}<br>⏰ Status: ${stop.wait_message}`;
                            marker.bindPopup(popupContent);
                            
                            console.log(`  📌 Adding marker to layer at position [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                            liveVehicleMarkersLayer.addLayer(marker);
                            
                            // === BEGIN: Dynamic Vehicle Animation Integration ===
                            // Find the polyline segment for this vehicle
                            const polylineSegment = findPolylineSegment(currentLinePaths, prevStop, stop);
                            const animationDuration = 10000; // 10 seconds for demo; adjust as needed
                            const vehicleId = stop.id + '-' + prevStop.id;
                            vehicleAnimations.set(vehicleId, {
                                marker: marker,
                                polyline: polylineSegment,
                                startTime: Date.now(),
                            ...
                            // === END: Dynamic Vehicle Animation Integration ===
                            
                            console.log(`  ✅ Vehicle ${vehicleIdx + 1} created and added to map`);
                        } else {
                            console.error(`  ❌ Failed to create vehicle icon for line ${lineNumber}`);
                        }
                    });
                    
                    console.log(`\n📊 SUMMARY:`);
                    console.log(`  - Total stops analyzed: ${data.line_stops_with_wait_times.length}`);
                    console.log(`  - Arriving stops found: ${arrivingStops.length}`);
                    console.log(`  - Vehicle markers created: ${vehicleAnimations.size}`);
                    console.log(`  - Highlight markers created: ${window._highlightedApproachingStops.length}`);
                    
                    // Check if any markers were actually added to the map
                    const totalMarkersOnMap = liveVehicleMarkersLayer.getLayers().length;
                    console.log(`  - Markers actually on map: ${totalMarkersOnMap}`);
                    
                    if (totalMarkersOnMap === 0) {
                        console.warn(`  ⚠️ NO MARKERS WERE ADDED TO THE MAP!`);
                        console.log(`  🔍 Debugging: Let's try to create a test marker...`);
                        
                        // Create a test marker to verify the system works
                        const testMarker = L.marker([45.4642, 9.1900], {
                            icon: L.divIcon({
                                className: 'test-vehicle-marker',
                                html: '🚌',
                                iconSize: [20, 20],
                                iconAnchor: [10, 10]
                            })
                        }).bindPopup('Test Vehicle Marker');
                        
                        liveVehicleMarkersLayer.addLayer(testMarker);
                        console.log(`  🧪 Test marker added. Total markers now: ${liveVehicleMarkersLayer.getLayers().length}`);
                    }
                    ...
                    // Show/hide zoom to vehicle button
                    const showZoomButton = arrivingStops.length > 0;
                    document.getElementById('zoom-to-vehicle').style.display = showZoomButton ? 'inline-block' : 'none';
                    console.log(`🔘 Zoom to vehicle button: ${showZoomButton ? 'SHOWN' : 'HIDDEN'}`);
                    
                } else {
                    console.warn('❌ No line_stops_with_wait_times data available');
                    document.getElementById('zoom-to-vehicle').style.display = 'none';
                }
            })
            .catch(error => {
  ...
  // === BEGIN: Dynamic Vehicle Animation Additions ===
  // Helper: Find the polyline segment between two stops
  ...
  // Helper: Interpolate position along a polyline segment
  ...
  // === END: Dynamic Vehicle Animation Additions ===
</script>
