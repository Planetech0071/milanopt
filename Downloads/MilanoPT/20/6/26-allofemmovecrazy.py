...
<script>
  ...
  // FIXED: Animation frame function with proper vehicle tracking
  function animateVehicles() {
      ...
      vehicleAnimations.forEach((animData, vehicleId) => {
          if (animData.isAnimating && animData.polyline && animData.marker) {
              ...
              // If animation is complete, move to next segment or loop
              if (progress >= 1) {
                  // For now, just restart the animation (loop)
                  animData.startTime = Date.now();
                  console.log(`🔄 Restarting animation for vehicle ${vehicleId}`);
              }
          }
      });
      
      // Continue animation loop if we have active animations
            ...
            .then((data) => {
                ...
                if (data.vehicles && data.vehicles.length > 0) {
                    ...
                    data.vehicles.forEach((vehicle, vehicleIdx) => {
                        ...
                        // Calculate vehicle position based on wait time
                        const stopLatLon = [vehicle.lat, vehicle.lon];
                        const prevLatLon = prevStop ? [prevStop.lat, prevStop.lon] : stopLatLon;
                        
                        // Calculate distance based on wait time and vehicle type
                        const distanceFromStop = calculateVehicleDistance(vehicle.raw_message, currentVehicleType);
                        console.log(`  📏 Distance from stop: ${distanceFromStop} meters`);
                        ...
                        if (currentLinePaths.length > 0) {
                            ...
                            const t = Math.min(distanceFromStop / 1000, 0.5); // Max 50% back
                            vehiclePosition = [
                        ...
                        if (vehicleIcon) {
                            ...
                            // FIXED: Create animation data with proper polyline segment
                            if (currentLinePaths.length > 0 && prevStop && currentStop) {
                                ...
                                const animationDuration = Math.max(vehicle.wait_time * 1000, 5000); // At least 5 seconds
                                ...
                                console.log(`  🎬 Animation created for vehicle ${vehicle.id} (duration: ${animationDuration}ms)`);
                            }
