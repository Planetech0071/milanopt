...
function animateVehicles() {
            ...
            .then((data) => {
                ...
                if (data.vehicles && data.vehicles.length > 0) {
                    ...
                    data.vehicles.forEach((vehicle, vehicleIdx) => {
                        ...
                        const currentStop = allStops[currentStopIdx];
                        const prevStop = allStops[prevStopIdx];
                        ...
                        // NEW: Spawn vehicles at the previous stop on the polyline route
                        const stopLatLon = [vehicle.lat, vehicle.lon];
                        let vehiclePosition;
                        
                        if (currentLinePaths.length > 0 && prevStop) {
                            // Find the actual previous stop on the polyline route
                            const actualPrevStop = findPreviousStopOnPolyline(currentLinePaths, stopLatLon, allStops);
                            if (actualPrevStop) {
                                vehiclePosition = [actualPrevStop.lat, actualPrevStop.lon];
                                console.log(`  📍 Vehicle spawned at previous stop: ${actualPrevStop.name} [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                            } else {
                                // Fallback to array-based previous stop
                                vehiclePosition = prevStop ? [prevStop.lat, prevStop.lon] : stopLatLon;
                                console.log(`  📍 Vehicle spawned at fallback previous stop: [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                            }
                        } else {
                            // Fallback if no polyline data
                            vehiclePosition = prevStop ? [prevStop.lat, prevStop.lon] : stopLatLon;
                            console.log(`  📍 Vehicle spawned at array-based previous stop: [${vehiclePosition[0]}, ${vehiclePosition[1]}]`);
                        }
                        ...
                        if (vehicleIcon) {
                            ...
                            // FIXED: Create animation data with better duration calculation
                            if (currentLinePaths.length > 0 && prevStop && currentStop) {
                                const polylineSegment = findPolylineSegment(currentLinePaths, prevStop, currentStop);
    
                                // Calculate animation duration based on wait time (more realistic)
                                let animationDuration;
                                if (vehicle.wait_time === 0) {
                                    animationDuration = 3000; // 3 seconds for "arriving" vehicles
                                } else if (vehicle.wait_time <= 2) {
                                    animationDuration = vehicle.wait_time * 2000; // 2 seconds per minute
                                } else {
                                    animationDuration = Math.min(vehicle.wait_time * 1000, 15000); // Max 15 seconds
                                }
                                ...
                                vehicleAnimations.set(vehicle.id, {
                                    ...
                                    isAnimating: true,
                                    stopName: vehicle.stop_name,
                                    prevStop: prevStop,
                                    currStop: currentStop,
                                    vehicleId: vehicle.id
                                });
    
                                console.log(`  🎬 Animation created for vehicle ${vehicle.id} (duration: ${animationDuration}ms, wait_time: ${vehicle.wait_time}min)`);
                            }
  ...
  // NEW: Helper function to find the actual previous stop on the polyline route
  function findPreviousStopOnPolyline(allPaths, targetStopLatLon, allStops) {
      ...
      console.log(`  🎯 Target stop found at path ${targetPathIdx}, point ${targetPointIdx}`);
      
      // Look backwards along the polyline to find the previous stop
      const targetPath = allPaths[targetPathIdx];
      let searchRadius = 0.01; // Start with small radius and expand if needed
      
      for (let attempts = 0; attempts < 3; attempts++) {
          for (let i = targetPointIdx - 1; i >= 0; i--) {
              ...
              for (const stop of allStops) {
                  ...
                  if (stopDistance < searchRadius) {
                      console.log(`  ✅ Found previous stop: ${stop.name} at distance ${stopDistance.toFixed(6)}`);
                      return stop;
                  }
              }
          }
          searchRadius *= 2; // Expand search radius and try again
          console.log(`  🔄 Expanding search radius to ${searchRadius.toFixed(6)}`);
      }
      
      console.warn('  ⚠️ No previous stop found on polyline route');
      return null;
  }
</script>
