...
<script>
  ...
  // Helper function to get direction from bearing angle
  ...
  // Helper function to get vehicle icon based on type and direction
  function getVehicleIcon(vehicleType, bearing) {
    ...
    return L.icon({
      iconUrl: basePath + direction + '.png',
        ...
        .then(data => {
          ...
          if (data.line_stops_with_wait_times && data.line_stops_with_wait_times.length > 0) {
            ...
            if (closestVehicleStop && minWaitMinutes !== Infinity) {
              ...
              // Create and add new marker with directional image
              const vehicleIcon = getVehicleIcon(currentVehicleType, bearing);
              ...
              if (data.vehicles && data.vehicles.length > 0) {
                ...
                data.vehicles.forEach(vehicle => {
                  if (closestVehicleStop && vehicle.stop_name === closestVehicleStop.name) {
                    console.log(`Skipping duplicate vehicle for stop: ${vehicle.stop_name}`);
                  ...
                  if (approachingStation) {
                    ...
                    // Create vehicle marker with directional image
                    const additionalVehicleIcon = getVehicleIcon(currentVehicleType, vehicleBearing);
                    const vehicleMarker = L.marker(projectedLatLon, { icon: additionalVehicleIcon });
